...
class FlightOrderHandlePage(WebPage):
    ...
    def change_offer_page(self):
        ...
        # 因部分航司基础数据可能未配置有网上退改签政策代码
        # self.js_click(handle["舱位代码"])
        # self.js_click(handle["第一个舱位"])
        self.input_text(handle["改签政策"], self.generate_change_Policy())
        self.input_text(handle["退票政策"], self.generate_return_Policy())
    ...
    def round_trip_ticket_confirm(self, order_id, task_type):
        ...
        if not self.to_task_detail_page(order_id, task_type):
            return False
        self.ticket_confirm_page()
        assert self.task_submit_result(), "没有调出任务按钮"
        self.js_click(handle["查询"])
    ...
    def generate_change_Policy(self):
        new_list = []
        for i in range(4):
            num = random.randint(1, 60)
            new_list.append(num)
        new_list.sort()
        return "航班规定离站时间前7天(168小时)(含)之前，{}%；航班规定离站时间前48小时(含)之前，{}%；航班规定离站时间前4小时(含)之前，{}%；航班规定离站时间前4小时之后，{}%。".format(
            new_list[0], new_list[1], new_list[2], new_list[3])

    def generate_return_Policy(self):
        new_list = []
        for i in range(4):
            num = random.randint(1, 60)
            new_list.append(num)
