...
class BookFlightPage(WebPage):
    ...
    def add_passenger_one_way(self, departure_city, arrive_city):
        log.info("添加乘机人")
        self.roll_to_tagert(book["添加乘机人"])
        self.js_click(book["添加乘机人"])
        try:
            element = self.find_element(book["余票不足弹窗"])  # 余票不足弹窗"可能会不存在
            self.element_js_click(element)
            log.info("关闭余票不足弹窗，并重新选择航班")
            self.roll_to_top()
            self.place_order(departure_city, arrive_city, one_person=True)
        except:
            log.info("余票充足")
            name = fake.name()
    ...
    def add_passenger_round_trip(self, departure_city, arrive_city):
        log.info("添加乘机人")
        self.roll_to_tagert(book["添加乘机人"])
        self.js_click(book["添加乘机人"])
        try:
            ...
            log.info("关闭余票不足弹窗，并重新选择航班")
            self.roll_to_top()
            self.book_round_trip(departure_city, arrive_city)
        except:
            log.info("余票充足")
            name = fake.name()
            log.info("输入姓名：{0}".format(name))
    ...
    def place_order(self, departure_city, arrive_city, one_person=True):
        ...
        while True:
            try:
                invalid_price_element = self.find_element(book["价格失效"])
                self.element_js_click(invalid_price_element)
                self.click_book()
                try:
                    if self.element_exist(book["价格失效"]) is False:
                        break
                except:
                    continue
            except:
        ...
        if not one_person:
            self.add_passenger_one_way(departure_city, arrive_city)
            self.add_passenger_one_way(departure_city, arrive_city)
    ...
    def book_round_trip(self, departure_city, arrive_city):
        ...
        while True:
            try:
                invalid_price_element = self.find_element(book["价格失效"])
                self.element_js_click(invalid_price_element)
                self.click_book("选择去程")
                try:
                    invalid_price_element = self.find_element(book["价格失效"])
                    if self.element_exist(invalid_price_element) is False:
                        break
                except:
                    continue
            except:
        ...
        while True:
            try:
                ...
                break
        self.passenger_by_add_staff()
        self.add_passenger_round_trip(departure_city, arrive_city)
        self.click_build_order()
        self.click_credit_pay()
        return self.get_pay_result()
